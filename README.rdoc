...
==== Geonames (<tt>:geonames</tt>)

API key:: (use a 'username' field)
Quota:: 30'000 credits daily limit per application (identified by the parameter 'username'), the hourly limit is 2000 credits. A credit is a web service request hit for most services. An exception is thrown when the limit is exceeded.
Region:: world
SSL support:: no
Languages:: English
Documentation:: http://www.geonames.org/export/web-services.html
Terms of Service:: free : GeoNames data is free. Datasets of lesser quality and coverage from other 'vendors' may cost more than 25.000$ per year.
